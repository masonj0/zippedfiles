...
async def run_unified_pipeline(config: Dict, args: Optional[argparse.Namespace]):
    """
    The main, unified data processing pipeline. It fetches data from both
    V2 adapters and V1 local files, merges them, and then scores and reports.
    """
    # 1. Get races from V2 adapters
    races_from_adapters = await run_v2_adapter_pipeline(config)

    # 2. Get races from V1 local files (run in executor to not block loop)
    loop = asyncio.get_running_loop()
    print("Parsing local HTML files...")
    # The parse_local_files function is synchronous, so we run it in an executor
    ...
    # 4. Score and generate reports
    print("Scoring races...")
    scored_results = score_races(final_races, config)
...
from enhanced_scanner import discover_rss, fetch_breadcrumb_page, scan_js_for_endpoints

# =============================================================================
# --- ADVANCED PREFETCH LOGIC ---
# =============================================================================

# Add your sources here or load from config.json
BASES = [
...
async def advanced_prefetch_once():
    ...
    logging.info("Starting one pass of advanced prefetch and discovery...")
    for source in BASES:
        base_url = source["base_url"]
        try:
            # Breadcrumb warm-up (homepage -> category -> detail)
            logging.info(f"[{base_url}] Warming up session with breadcrumb navigation...")
            # Construct a path that handles an empty detail page
            path_parts = [p for p in [source["category"], source["detail"]] if p]
            html = await fetch_breadcrumb_page(base_url, *path_parts)

            # JS endpoint discovery
            logging.info(f"[{base_url}] Scanning for JS endpoints...")
            endpoints = await scan_js_for_endpoints(html, base_url)
            if endpoints:
                logging.info(f"[{base_url}] Discovered endpoints: {endpoints}")

            # RSS discovery
            logging.info(f"[{base_url}] Discovering RSS feeds...")
            rss_links = await discover_rss(base_url)
            if rss_links:
                logging.info(f"[{base_url}] Discovered RSS feeds: {rss_links}")

            # TODO: enqueue endpoints and RSS links into your existing parsing queue
...
def advanced_prefetch_menu_action():
    ...
    try:
        # This will run indefinitely in bursts until cancelled with Ctrl+C
        safe_async_run(run_bursts(advanced_prefetch_once), "Advanced Prefetch Discovery")
    finally:
        # Ensure the client is closed when the user stops the process
...
def main_menu(config: Dict):
    ...
    while True:
        ...
        print(" 7. Run Advanced Prefetch & Discovery")
        ...
        if choice == '1':
            safe_async_run(run_unified_pipeline(config, None), "Unified Analysis")
        elif choice == '2':
            run_batch_parse(config, None) # This is the standalone V1 file parser
        elif choice == '3':
            run_persistent_engine(config, argparse.Namespace()) # Use default args
        elif choice == '4':
            ...
            # Simplified view/validate logic
...
def main_cli(config: Dict, args: argparse.Namespace):
    ...
    if args.command == 'analyze':
        safe_async_run(run_unified_pipeline(config, args), "Unified Analysis")
