...
class SkySportsAdapter(BaseAdapterV3):
    ...
    def _parse_races(self, soup: BeautifulSoup) -> list[RawRaceDocument]:
        ...
        logging.info(f"[{self.source_id}] Found {len(meeting_containers)} potential race containers.")
        ...
        for container in meeting_containers:
            try:
                ...
                details_parts = [span.get_text(strip=True) for span in container.find_all('span', class_=['sdc-site-racing-meetings__event-name', 'sdc-site-racing-meetings__event-details'])]
                ...
                race_doc = RawRaceDocument(
                    ...
                    extras={
                        ...
                        "discipline": FieldConfidence(map_discipline(details_text), 0.5, self.source_id)
                    }
                )
