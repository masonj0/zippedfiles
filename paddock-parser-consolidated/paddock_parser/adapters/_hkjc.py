...
from ..normalizer import canonical_track_key, canonical_race_key, parse_hhmm_any
...
class HkjcAdapter(BaseAdapterV3):
    ...
    async def fetch(self) -> list[RawRaceDocument]:
        ...
        target_url = self.site_config.get(
        ...
        if not await bootstrap_session_with_playwright(target_url):
            ...
            logging.warning(
                f"[{self.source_id}] Playwright bootstrap failed. Proceeding with simple fetch..."
            )
    ...
    def _parse_races(self, soup: BeautifulSoup) -> list[RawRaceDocument]:
        ...
        if not race_table:
            logging.warning(
                f"[{self.source_id}] Could not find main racecard table with class 'racecard'."
            )
            return []
        ...
        for i, row in enumerate(race_rows):
            try:
                ...
                race_time = (
                    parse_hhmm_any(time_el.get_text(strip=True)) if time_el else f"12:{i * 15:02d}"
                )
                ...
                races.append(
                    RawRaceDocument(
                        ...
                        extras={
                            "race_title": FieldConfidence(f"Race {race_no}", 0.5, self.source_id)
                        },
                    )
                )
            except Exception as e:
