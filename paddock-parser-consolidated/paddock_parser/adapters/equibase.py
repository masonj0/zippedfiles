...
from ..normalizer import NormalizedRace, canonical_track_key, canonical_race_key
...
class EquibaseAdapter(BaseAdapterV3):
    ...
    def initialize(self) -> bool:
        ...
        print(
            f"--- [EquibaseAdapter] super().initialize() returned: {initialized} ---",
            file=sys.stderr,
        )
    ...
    async def fetch(self) -> list[RawRaceDocument]:
        ...
        try:
            ...
            list_response = await resilient_get(
                self._entries_url, extra_headers=config.get("StealthHeaders")
            )

            if not list_response or not list_response.text:
                logging.error(
                    f"[{self.source_id}] Failed to retrieve content from {self._entries_url}"
                )
                return []
        ...
        if race_urls:
            logging.info(
                f"[{self.source_id}] Successfully fetched race list. URLs found: {race_urls[:5]}"
            )
    ...
    def _parse_racecard(self, html_content: str) -> list[NormalizedRace]:
        ...
        track_name = (
            track_name_elem.text.strip().replace(" Entries", "")
            if track_name_elem
            else "Unknown Track"
        )
