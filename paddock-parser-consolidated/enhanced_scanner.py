...
async def scan_js_for_endpoints(html: str, base_url: str) -> List[str]:
    if not FEATURES.get("enable_js_endpoint_scan"):
        return []
    # This regex needs to be applied to the full HTML content, including script tags.
...
async def fetch_breadcrumb_page(base_url: str, *path_parts: str) -> str:
    """
    Navigates through a sequence of URL path parts, building on the base_url.
    e.g., fetch_breadcrumb_page("https://a.com", "b", "c") will visit:
    1. https://a.com
    2. https://a.com/b
    3. https://a.com/b/c
    """
    ...
    # We only need to visit the sequence, starting from the second url in the list
    # as the first one is just the base. The breadcrumb_get will handle the referrers.
