...
try:
    from normalizer import NormalizedRace, NormalizedRunner, normalize_course_name, parse_hhmm_any, convert_odds_to_fractional_decimal
    from analysis import V2Scorer
    from fetching import resilient_get, close_shared_async_client
except ImportError:
    print("FATAL: Ensure normalizer.py, analysis.py and fetching.py are in the same directory.", file=sys.stderr)
    sys.exit(1)
...
async def maybe_start_webhook(config: Dict[str, Any]):
    ...
    await site.start()
    logging.info(f"Webhook server started on http://{WH.get('host','0.0.0.0')}:{WH.get('port',8765)}{WH.get('path','/trigger')}")
    return runner
        ...
        try:
            ...
            for race_el in race_elements:
                ...
                if field_size > 0:
                    ...
                    runners.append(NormalizedRunner(runner_id="fav", name="Simulated Fav", saddle_cloth="1", odds_decimal=2.0))
                    runners.append(NormalizedRunner(runner_id="2nd-fav", name="Simulated 2nd Fav", saddle_cloth="2", odds_decimal=4.0))
                    for i in range(2, field_size):
                         runners.append(NormalizedRunner(runner_id=f"runner-{i+1}", name=f"Runner {i+1}", saddle_cloth=str(i+1), odds_decimal=10.0 + i))
...
async def main_loop(config: Dict, scorer: V2Scorer, alerted_ids: Set[str]):
    """The main asynchronous event loop for the mobile engine."""
    global SCAN_NOW

    runner = await maybe_start_webhook(config)
    check_interval = config.get("MONITORING_LOOP", {}).get("CHECK_INTERVAL_SECONDS", 900)

    try:
        # Run only once for testing
        for _ in range(1):
            await perform_scan_and_alert(config, scorer, alerted_ids)

            logging.info(f"Test cycle complete. Not waiting.")

            # Wait for the interval, but wake up immediately if the webhook is triggered
            try:
                # This creates a cancellable sleep
                sleep_task = asyncio.create_task(asyncio.sleep(check_interval))

                # Wait for either the sleep to finish or SCAN_NOW to be set
                while not sleep_task.done() and not SCAN_NOW:
                    await asyncio.sleep(0.5) # Check every half a second

                if SCAN_NOW:
                    logging.info("Webhook triggered! Starting scan immediately.")
                    SCAN_NOW = False # Reset the flag
                    sleep_task.cancel() # Cancel the sleep

            except asyncio.CancelledError:
                pass # Expected when webhook is triggered
...
def main():
    ...
    try:
        asyncio.run(main_loop(config, scorer, alerted_race_ids))
