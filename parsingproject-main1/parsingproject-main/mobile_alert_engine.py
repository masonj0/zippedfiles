...
def load_alert_state(state_file_path: Path) -> Set[str]:
    ...
    if not state_file_path.exists(): return set()
    try:
    ...
    for container in meeting_containers:
        try:
            ...
            for race_el in race_elements:
                ...
                if not race_time_parsed: continue
                ...
                if runners_el:
                    ...
                    if runners_match: field_size = int(runners_match.group(1))
...
async def send_webhook_notification(config: Dict, title: str, content: str):
    ...
    if not webhook_config.get("enabled"): return
